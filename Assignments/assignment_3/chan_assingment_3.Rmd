...
possible_cars$power = fuel_effic(V=speed_base, m=possible_cars$mass, A=possible_cars$area)
...
# using a for loop to calculate the mean power for each car 
  ...
  power_all_cars_mean[,i] = fuel_effic(A=possible_cars$area[i], m=possible_cars$mass[i], V=speeds)
   }
...
for (i in 1:nrow(possible_cars)) {
  power_all_cars_max[, i] = fuel_effic(V = max(speeds), m = possible_cars$mass[i], A = possible_cars$area[i])
}
...
power_all_cars_mean2 = pmap(list(A = possible_cars$area, m=possible_cars$mass), V=speeds, fuel_effic)
power_all_cars_mean2
...
```{r}
# pmap runs a function for each value in a list of parameters, with other parameters set for each iteration
power_all_cars_max2 = pmap(list(A = possible_cars$area, m=possible_cars$mass), fuel_effic, V=max(speeds))
...
grid.arrange(for_mean_plot, pmap_plot, ncol=2)
```
